...
class DefaultParametersTest extends AnyFunSuite with Matchers with SwaggerSpecRunner {
  ...
  def testVersion(version: ZIOHttpVersion): Unit =
    test(s"$version - Ensure responses are generated") {
      ...
      def apply[F[_]](host: String = "http://petstore.swagger.io")(implicit F: Async[F], httpClient: ZIOHttpClient[F]): StoreClient[F] = new StoreClient[F](host = host)(F = F, httpClient = httpClient)
      def httpClient[F[_]](httpClient: ZIOHttpClient[F], host: String = "http://petstore.swagger.io")(implicit F: Async[F]): StoreClient[F] = new StoreClient[F](host = host)(F = F, httpClient = httpClient)
    }"""

      val clientClass = q"""
    class StoreClient[F[_]](host: String = "http://petstore.swagger.io")(implicit F: Async[F], httpClient: ZIOHttpClient[F]) {
      val basePath: String = ""
