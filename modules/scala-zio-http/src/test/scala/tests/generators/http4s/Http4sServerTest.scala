...
class ZIOHttpServerTest extends AnyFunSuite with Matchers with SwaggerSpecRunner {
  ...
  def testVersion(version: ZIOHttpVersion): Unit = {
    test(s"$version - Ensure routes are generated") {
      ...
      trait StoreHandler[F[_]] {
        ...
        def getFooBar(respond: StoreResource.GetFooBarResponse.type)(bar: Long): F[StoreResource.GetFooBarResponse]
    ...
    test(s"$version - Ensure routes are generated with tracing") {
      val (
        ...
        _,
        Servers(Server(_, _, genHandler, genResource :: _) :: Nil, Nil)
      ) = runSwaggerSpec(scalaInterpreter)(swagger)(Context.empty.copy(tracing = true), version.value)
      ...
      trait StoreHandler[F[_]] {
        ...
        def getFoo(respond: StoreResource.GetFooResponse.type)()(traceBuilder: TraceBuilder[F]): F[StoreResource.GetFooResponse]
        def getFooBar(respond: StoreResource.GetFooBarResponse.type)(bar: Long)(traceBuilder: TraceBuilder[F]): F[StoreResource.GetFooBarResponse]
    ...
    test(s"$version - Ensure routes are generated with custom extraction") {
      val (
        ...
        _,
        Servers(Server(_, _, genHandler, genResource :: _) :: Nil, Nil)
      ) = runSwaggerSpec(scalaInterpreter)(swagger)(Context.empty.copy(customExtraction = true), version.value)
      val handler =
        q"""
      trait StoreHandler[F[_], -E] {
        ...
        def getFoo(respond: StoreResource.GetFooResponse.type)()(extracted: E): F[StoreResource.GetFooResponse]
        def getFooBar(respond: StoreResource.GetFooBarResponse.type)(bar: Long)(extracted: E): F[StoreResource.GetFooBarResponse]
