...
class BasicTest extends AnyFunSuite with Matchers with SwaggerSpecRunner {
  ...
  def testVersion(version: ZIOHttpVersion): Unit = {
    ...
    test(s"$version - Properly handle all methods") {
      ...
      def apply[F[_]](host: String = "http://localhost:1234")(implicit F: Async[F], httpClient: ZIOHttpClient[F]): Client[F] = new Client[F](host = host)(F = F, httpClient = httpClient)
      def httpClient[F[_]](httpClient: ZIOHttpClient[F], host: String = "http://localhost:1234")(implicit F: Async[F]): Client[F] = new Client[F](host = host)(F = F, httpClient = httpClient)
    }"""
      val client = q"""
    class Client[F[_]](host: String = "http://localhost:1234")(implicit F: Async[F], httpClient: ZIOHttpClient[F]) {
      val basePath: String = ""
